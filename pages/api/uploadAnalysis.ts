...
const handleFileUpload = async (
  ...
  try {
    ...
    if (
      ...
      uploadResults.error.message !== 'The resource already exists'
    ) {
...
export async function analyzeFile(imageUrl: string, specialInstructions: string): Promise<any> {
    ...
    'Analyze a CO₂ emissions report based on the image provided. Don’t guess; leave blank if information is not available. All emission values should be converted to whole tonnes of CO₂ equivalents (t CO₂e). Convert from thousand tonnes and million tonnes (Mt or mt) to whole tonnes (t) if needed.',
    {
      yearlyReports: {
        ...
        items: {
          ...
          required: ['all_cats', 'scope_1', 'cat_1'],
        },
      },
    },
    ['yearlyReports'],
    ...
    [
      ...
      },
      {
        ...
        content: [
          {
            type: 'text',
            text: `Help me analyze the company emissions report in this image. ${specialInstructions}`,
          },
  ...
  const results =
    completion.tool_calls?.[0].function.arguments !== undefined
...
const YEARLY_REPORT_FIELDS: Record<string, OpenAIFunctionParameterItems> = {
  ...
  scope_2_market_based: {
    type: 'number',
    description:
      'Indirect emissions from the generation of purchased electricity, heat, and steam, based on market-based methods',
  },
  scope_2_location_based: {
    type: 'number',
    description:
      'Indirect emissions from the generation of purchased electricity, heat, and steam, based on location-based methods',
  },
  scope_2_unknown: {
    type: 'number',
    description: 'Indirect emissions from purchased electricity with an unspecified method',
  },
  total_scope_3: { type: 'number', description: 'Total reported emissions for Scope 3' },
  total_emission_market_based: {
    type: 'number',
    description: 'Total emissions based on market-based methods',
  },
  total_emission_location_based: {
    type: 'number',
    description: 'Total emissions based on location-based methods',
  },
  ...
  cat_3: {
    type: 'number',
    description:
      'Emissions in Category 3: Fuel- and energy-related activities not included in Scope 1 or 2',
  },
  cat_4: {
    type: 'number',
    description: 'Emissions in Category 4: Upstream transportation and distribution',
  },
  ...
  cat_9: {
    type: 'number',
    description: 'Emissions in Category 9: Downstream transportation and distribution',
  },
  ...
  cat_12: {
    type: 'number',
    description: 'Emissions in Category 12: End-of-life treatment of sold products',
  },
  ...
  share_upstream_of_scope_3: {
    type: 'number',
    description: '% share of total upstream emissions in Scope 3',
  },
  scope_1_share_of_total_upstream_emissions: {
    type: 'number',
    description: '% share of Scope 1 emissions in total upstream emissions',
  },
  total_upstream_emissions: {
    type: 'number',
    description: 'Total upstream emissions in CO₂ equivalents',
  },
  ...
  source_emissions_page_move: {
    type: 'integer',
    description: 'Report page number for emissions data',
  },
  ...
  publication_date: {
    type: 'string',
    description: 'Date of publication of the data, format YYYY-MM-DD',
  },
  comment: { type: 'string', description: 'Additional comments or notes' },
  status: {
    type: 'string',
    description: 'Status of the data or report',
    enum: ['Not Started', 'Ongoing', 'Done'],
  },
};
