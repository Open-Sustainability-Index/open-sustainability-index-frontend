  ...
  if (req.method === 'POST') {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        ...
        res.status(200).json({ message: 'File uploaded successfully', analysis: analysis })
      } catch (error) {
...
const REPORT_FIELDS: Record<string, OpenAIFunctionParameterItems> = {
  ...
  scope_2_market_based: { type: 'string', description: 'Indirect emissions from the generation of purchased electricity, heat, and steam, based on market-based methods' },
  scope_2_location_based: { type: 'string', description: 'Indirect emissions from the generation of purchased electricity, heat, and steam, based on location-based methods' },
  scope_2_unknown: { type: 'string', description: 'Indirect emissions from purchased electricity with an unspecified method' },
  total_scope_3: { type: 'string', description: 'Total other indirect emissions not covered in Scope 2' },
  total_emission_market_based: { type: 'string', description: 'Total emissions based on market-based methods' },
  total_emission_location_based: { type: 'string', description: 'Total emissions based on location-based methods' },
  ...
  total_upstream_emissions: { type: 'number', description: 'Total upstream emissions in CO₂ equivalent' },
  ...
  emission_intensity: { type: 'number', description: 'Emission intensity metric' },
...
export async function analyzeFile (file: OpenAI.Files.FileObject): Promise<void> {
  const analysisFunction = createFunction(
    ...
    'Analyze a CO₂ emissions report based on the file provided. Leave blank if information is not available.',
    {
      yearlyReports: {
