...
function CompanyListPage({
  ...
  detailPageLink,
  filters,
}: CompanyListPageProps): React.ReactElement {
  return (
    <>
      <PageTopBanner
        subtitle='Companies'
        title={filters?.tags?.toUpperCase() || 'Find any company'}
      />
      <CompanyList companies={companies} page={page} detailPageLink={detailPageLink} />
    </>
  );
}
...
export const getCompaniesListProps = async (
  params: ListEndpointParams,
  context: GetServerSidePropsContext<CompanyListPageParams>,
): Promise<GetServerSidePropsResult<CompanyListPageProps>> => {
