    ...
    return (
      <Html lang={config.locale?.split('_')[0]}>
        <Head>
          <link
            rel='stylesheet'
            href={`https://fonts.googleapis.com/css2?${config.fonts?.map(([fontName, fontWeight]) => `family=${`${fontName.replace(/ /g, '+')}${fontWeight !== undefined ? ':' + fontWeight : ''}`}`).join('&') ?? ''}&display=swap`}
          />
          ...
          {config.googleAnalyticsId !== undefined ? (
            <>
              <script
                async
                src={`https://www.googletagmanager.com/gtag/js?id=${config.googleAnalyticsId as string}`}
              />
              <script
