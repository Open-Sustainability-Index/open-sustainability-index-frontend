...
export const companiesPageProps = (
  ...
  options?: ListEndpointParams,
): PageProps => {
  const first3CompanyNames = companies
    .slice(0, 3)
    .map((company) => titleCase(company.company_name))
    .join(', ');
  const filterTags =
    options?.filters?.tags !== undefined ? `${options?.filters?.tags?.toUpperCase()} ` : '';
  const forCompanies = `for ${filterTags}companies`;
  return {
    title:
      'Company GHG emission data ' +
      ((filterTags !== '' ? forCompanies : undefined) ?? 'per industry and year'),
...
export const fetchCompanies = async ({
  ...
  order = 'asc',
  filters,
}: ListEndpointParams): Promise<CompaniesCompany[]> => {
...
export const CompaniesContextProvider = (props: CompaniesInputProps): React.ReactElement => {
  ...
  return (
    <CompaniesContext.Provider value={companiesContext}>{props.children}</CompaniesContext.Provider>
  );
};
...
export const companyPath = (company: Company | CompaniesCompany): string =>
...
export const fetchCompany = async (companySlug: string): Promise<Company> => {
  const results = await makeRestRequest(
    'GET',
...
export const fetchCompanyHistory = async (companySlug: string): Promise<Company> => {
  const results = await makeRestRequest(
    'GET',
    `companies/${companySlug}/all-years`,
    undefined,
    process.env.BACKEND_API_KEY,
  );
  return results?.data;
