  ...
  const handleRowClick = (row: DataTableRow): void => {
    if (row[rowKeyField as keyof typeof row] !== undefined && detailPageLink !== undefined) {
      const newPath = (
        detailPageLink +
        '/' +
        (row[rowKeyField as keyof typeof row] as string)
  ...
  const handleSortColumn = (event: any, sort: string, order: 'asc' | 'desc'): void => {
    if (detailPageLink !== undefined) {
      const newPath =
        detailPageLink +
        '?' +
        changeQueryString(changeQuery(router.query, 'sort', sort), 'order', order);
      void router.push(newPath);
    }
  };
        ...
        <Table aria-label='simple table'>
          ...
          <TableBody>
            {data.map((row, index) => (
              ...
              >
                {headersToDisplay.map((header, index) => (
                  <DataTableCell key={index} index={index} row={row} header={header} />
                ))}
              </TableRow>
            ))}
          </TableBody>
  ...
  return (
    <TableCell
      ...
      sx={{
        ...
        '&:not(:first-child)': { width: 150, textAlign: 'right' },
...
const DataTableCell = ({ index, row, header, align }: DataTableCellProps): React.ReactElement => (
  ...
  >
    {header.type === 'status' ? (
      ...
      row[header.field] !== undefined && (
        <Chip label={row[header.field]} color={row[header.statusField as keyof typeof row]} />
      )
    ) : header.type === 'link' ? (
        ...
        <TableBody>
          ...
          {headers.slice(1).map((header, index) => (
            <TableRow
              key={index}
              sx={
                header.isHorizontalHeader === true ? { backgroundColor: COLORS.GRAY_LIGHT } : null
              }
            >
