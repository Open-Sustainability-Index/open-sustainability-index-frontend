  ...
  const ambitionAndDevelopment = lastEmission !== undefined
    ? [
        (targetNetZero !== undefined || targetNearTerm !== undefined)
          ...
          : '',
    `In ${lastEmission?.year as string}, ${companyName} reported a total of ${formatAmount(lastEmission?.total_reported_emission_scope_1_2_3)} ton CO₂e, and a net revenue of ${formatAmount(lastEmission?.revenue)} M USD, resulting in a emissions intensity of ${formatAmount(intensity)} t CO₂e / M USD.`
...
const emissionsOverviewHeaders: EmissionsDataTableHeader[] = [
  { field: 'year', label: 'Year', type: 'number', align: 'right' },
  { field: 'total_emission_market_based', label: 'Total Emissions - Market Based (t CO₂e)', type: 'number', isEditable: false, align: 'right', format: (value: number): string => formatAmount(value) },
  { field: 'emission_intensity', label: 'Emissions Intensity (t CO₂e / M USD)', type: 'number', isEditable: false, align: 'right', format: (value: number): string => formatAmount(value) },
  { field: 'cradle_to_gate', label: 'Cradle-to-gate Intensity (t CO₂e / M USD)', type: 'number', isEditable: false, align: 'right', format: (value: number): string => formatAmount(value) }
]
const emissionsDetailsHeaders: EmissionsDataTableHeader[] = [
  ...
  { field: 'just_header', label: 'Upstream Emissions (t CO₂e)', isHorizontalHeader: true, isEditable: false, align: 'right' },
  ...
  { field: 'just_header', label: 'Downstream Emissions (t CO₂e)', isHorizontalHeader: true, isEditable: false, align: 'right' },
  ...
  { field: 'total_reported_emission_scope_1_2_3', label: 'Total Emissions (t CO₂e)', isHorizontalHeader: true, isEditable: false, type: 'number', align: 'right', format: (value: number): string => formatAmount(value) },
