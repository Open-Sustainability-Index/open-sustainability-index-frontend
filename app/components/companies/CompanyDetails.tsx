...
const CompanyDetails = ({
  company = DEFAULT_COMPANY as Company,
  loading = false,
}: {
  company?: Company;
  loading: boolean;
      ...
      ? [
          targetNetZero !== undefined || targetNearTerm !== undefined
            ? [
                `${companyName} has set`,
                [
                  ...(targetNetZero !== undefined
                    ? [`a Net Zero-target for ${targetNetZero?.target_year},`]
                    : []),
                  ...(targetNearTerm !== undefined
                    ? [
                        `a near-term target to reduce it’s scope 1+2+3 emissions with ${targetNearTerm?.target_value} to ${targetNearTerm?.target_year}, from its base year ${targetNearTerm?.base_year}.`,
                      ]
                    : []),
                ].join(' and '),
              ].join(' ')
    ...
    <DataTableHorizontal
      ...
      headers={[
        { field: 'year', label: 'Year', align: 'right' },
        {
          ...
          label: 'Total Emissions - Market Based (t CO₂e)',
          align: 'right',
          format: (value: number): string => formatAmount(value),
        },
        {
          field: 'emission_intensity',
          label: 'Emissions Intensity (t CO₂e / M USD)',
          align: 'right',
          format: (value: number): string => formatAmount(value),
        },
        {
          field: 'cradle_to_gate:',
          label: 'Cradle-to-gate Intensity (t CO₂e / M USD)',
          align: 'right',
          format: (value: number): string => formatAmount(value),
        },
      ]}
