  ...
  const ambitionAndDevelopment = lastEmission !== undefined
    ? [
        (targetNetZero !== undefined || targetNearTerm !== undefined)
          ...
          : '',
    `In ${lastEmission?.year as number}, ${companyName} reported a total of ${lastEmission?.total_reported_emission_scope_1_2_3?.toLocaleString('en') as string} ton CO₂e, and a net revenue of ${lastEmission?.revenue?.toLocaleString('en') as string} M USD, resulting in a emissions intensity of ${intensity?.toLocaleString('en') as string} t CO₂e / M USD.`
