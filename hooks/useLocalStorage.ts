...
type LocalStorageReturnProps<T> = [T | undefined, (propertyValue: T) => void];

export default function useLocalStorage<T>(
  propertyName: string,
  defaultValue?: T,
): LocalStorageReturnProps<T> {
  ...
  const setValueInLocalStorage = (propertyValue: any): void => {
    ...
    const propertyValueObject =
      typeof propertyValue === 'object' ? JSON.stringify(propertyValue) : propertyValue;
