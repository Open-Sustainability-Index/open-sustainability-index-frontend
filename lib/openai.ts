...
export const getChatCompletion = async (
  ...
  console.log(`\nUsage tokens: ${completion.usage?.prompt_tokens as number}/${completion.usage?.completion_tokens as number}, est. cost $${((completion.usage?.prompt_tokens ?? 0 as number) * 0.005 / 1000 + (completion.usage?.completion_tokens ?? 0) * 0.015 / 1000).toFixed(4)}`)
